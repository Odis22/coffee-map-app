...
2. Once in the correct folder, run `git clone https://github.com/scolsen/coffee-map-app.git`

Once this is done you'll have a copy of the app's source on your machine. I've included the build process I've used in the app. You should have both a `package.json` file and a `gulpfile.js` that are utilized for the process. To build and run the app quickly: 

1. cd into `#pathToTheAppOnYourDevice` if you are not already in the folder. 
2. run `npm install` this will install the needed packages for building the app. 
3. run `gulp` running gulp will run the default gulp task which builds files and starts a server. 
4. In a browser of your choice, browse to `localhost:4000` the app should be running. 
5. To stop the server enter `control + c` in your terminal.

You can also check out the gulpfile to see individual gulp tasks and run them as you please. 

Check in the `src` folder to view source files. These files are built into minified versions in both `src` and `dest`, but unlike `src` `dest` contains no source files and only contains minified versions. The app is served from the `dest` files.

You can modify the `webserver` task in `gulpfile.js` to customize the way the app is served.  
